                         ...
                         neat_error_code code)
{
    ...
    switch (flow->socket->stack) {
        ...
        case NEAT_STACK_SCTP:
            ...
            neat_log(NEAT_LOG_INFO, "%s - SCTP - available streams: %d", __func__, flow->socket->sctp_streams_available);
#endif // defined(IPPROTO_SCTP) && defined(SCTP_STATUS) && !defined(USRSCTP_SUPPORT)
            break;
    ...
    neat_log(NEAT_LOG_INFO, "%s - Connected: %s/%s", __func__, proto, (flow->socket->family == AF_INET ? "IPv4" : "IPv6"));
...
static void io_writable(neat_ctx *ctx, neat_flow *flow, neat_error_code code)
{
    neat_log(NEAT_LOG_DEBUG, "%s", __func__);
...
{
    uint32_t n, i;
    struct neat_flow *reset_flow;

    n = (strrst->strreset_length - sizeof(struct sctp_stream_reset_event)) / sizeof(uint16_t);

    neat_log(NEAT_LOG_DEBUG, "%s - Stream reset event: flags = %x, ", strrst->strreset_flags, __func__);
    if (!((strrst->strreset_flags & SCTP_STREAM_RESET_INCOMING_SSN) && (strrst->strreset_flags & SCTP_STREAM_RESET_OUTGOING_SSN))) {
        neat_log(NEAT_LOG_ERROR, "%s - outgoing AND incomig reset required!", __func__);
    }
                       ...
                       neat_error_code code)
{
    ...
    struct neat_flow *flow_temp = NULL;
    ...
    if ((neat_base_stack(flow->socket->stack) == NEAT_STACK_SCTP) &&
        ...
        if (rcv_stream_id > 0 && socket->sctp_neat_peer) {
            if ((flow_temp = neat_sctp_get_flow_by_sid(socket, rcv_stream_id)) == NULL) {
                ...
                new_flow->multistream_id = rcv_stream_id;
...
static void
io_all_written(neat_ctx *ctx, neat_flow *flow)
{
    neat_log(NEAT_LOG_DEBUG, "%s", __func__);
    ...
    if (TAILQ_EMPTY(&flow->bufferedMessages) && code == NEAT_OK && amt > 0) {
        ...
        if (neat_base_stack(flow->socket->stack) == NEAT_STACK_SCTP) {
            ...
            if (flow->multistream_id) {
                sndinfo->snd_sid = flow->multistream_id;
            }
            ...
            if (flow->multistream_id) {
                sndrcvinfo->sinfo_stream = flow->multistream_id;
            }
#if defined(SCTP_EOR)
            if ((flow->isSCTPExplicitEOR) && (len == amt)) {
                sndrcvinfo->sinfo_flags |= SCTP_EOR;
...
neat_connect(struct neat_he_candidate *candidate, uv_poll_cb callback_fx)
{
    ...
    if (candidate->pollable_socket->stack == NEAT_STACK_SCTP) {
        ...
        init.sinit_num_ostreams = NEAT_SCTP_MAX_STREAMS;
        init.sinit_max_instreams = NEAT_SCTP_MAX_STREAMS;
                       ...
                       int socket_type)
{
    ...
    case NEAT_STACK_SCTP:
        ...
        initmsg.sinit_num_ostreams = NEAT_SCTP_MAX_STREAMS;
        initmsg.sinit_max_instreams = NEAT_SCTP_MAX_STREAMS;
        ...
        neat_log(NEAT_LOG_DEBUG, "Offering %d SCTP streams in/out", NEAT_SCTP_MAX_STREAMS);
#endif // defined(SCTP_INITMSG) && !defined(USRSCTP_SUPPORT)
...
neat_shutdown_via_kernel(struct neat_ctx *ctx, struct neat_flow *flow)
{
    ...
    if (flow->socket->multistream) {
        ...
        LIST_FOREACH(flow_itr, &flow->socket->sctp_multistream_flows, next_multistream_flow) {
...
neat_sctp_get_flow_by_sid(struct neat_pollable_socket *socket, uint16_t sid)
{
    ...
    LIST_FOREACH(flow, &socket->sctp_multistream_flows, next_multistream_flow) {
...
neat_sctp_reset_stream(struct neat_pollable_socket *socket, uint16_t sid)
{
    ...
    neat_log(NEAT_LOG_DEBUG, "%s - resetting stream %d", __func__, sid);
