        ...
        struct neat_event_cb *cb)
{
    ...
    for (cb_itr = cb_list_head->lh_first; cb_itr != NULL;
        ...
        if (cb_itr == cb) {
            //TODO: Debug level
            neat_log(NEAT_LOG_INFO, "%s - Callback for %u has already been added", __FUNCTION__, event_type);
            return RETVAL_FAILURE;
        }
    }

    //TODO: Debug level
    neat_log(NEAT_LOG_INFO, "%s - Added new callback for event type %u", __FUNCTION__, event_type);
        ...
        struct neat_event_cb *cb)
{
    ...
    if (cb_itr) {
        ...
        neat_log(NEAT_LOG_INFO, "%s - Removed callback for type %u", __FUNCTION__, event_type);
...
#if defined(USRSCTP_SUPPORT)
static void usrsctp_free(neat_flow *flow)
{
    neat_log(NEAT_LOG_DEBUG, "%s", __FUNCTION__);
...
void neat_free_final_flow(neat_flow *flow)
{
    neat_log(NEAT_LOG_DEBUG, "%s", __FUNCTION__);
                     ...
                     unsigned char *buffer, uint32_t amt, uint32_t *actualAmt)
{
    ...
    if (flow->sockProtocol == IPPROTO_SCTP) {
        ...
        if (flow->readBufferSize > amt) {
            neat_log(NEAT_LOG_DEBUG, "%s: Message too big", __FUNCTION__);
            return NEAT_ERROR_MESSAGE_TOO_BIG;
        }
...
neat_listen_via_kernel(struct neat_ctx *ctx, struct neat_flow *flow)
{
    ...
    if ((flow->fd = socket(flow->family, flow->sockType, flow->sockProtocol)) < 0) {
        neat_log(NEAT_LOG_ERROR, "%s: opening listening socket failed - %s", __FUNCTION__, strerror(errno));
        return -1;
    }
    len = (socklen_t)sizeof(int);
