...
#if 1
    neat_request_property(flow, "IPV4_REQUIRED");
    neat_request_property(flow, "IPV4_REQUIRED");
    neat_request_property(flow, "TCP_REQUIRED");
    neat_request_property(flow, "CONGESTION_CONTROL_REQUIRED");

    if (neat_check_property(flow, "IPV4_REQUIRED") != 1) {
        fprintf(stderr, "%s - error: neat_check_property: expected IPV4_REQUIRED to be set, but it was not\n", __func__);
        result = EXIT_FAILURE;
        goto cleanup;
    }

    if (neat_check_property(flow, "TCP_REQUIRED") != 1) {
        fprintf(stderr, "%s - error: neat_check_property: expected TCP_REQUIRED to be set, but it was not\n", __func__);
        result = EXIT_FAILURE;
        goto cleanup;
    }

    neat_remove_property(flow, "IPV4_REQUIRED");

    if (neat_check_property(flow, "IPV4_REQUIRED") != 0) {
        fprintf(stderr, "%s - error: neat_check_property: expected IPv4_REQUIRED to not be set, but it was\n", __func__);
        result = EXIT_FAILURE;
        goto cleanup;
    }
